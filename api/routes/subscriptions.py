...
async def subscription_status(
    ...
    try:
        ...
        # The subscription_data already contains the card details at the root level
        response = {
            'subscription_status': subscription_data.get('subscription_status', 'none'),
            'card_last4': subscription_data.get('card_last4'),
            'card_brand': subscription_data.get('card_brand'),
            'card_exp_month': subscription_data.get('card_exp_month'),
            'card_exp_year': subscription_data.get('card_exp_year')
        }      
        return response
    except Exception as e:
        logger.error(f"Error in subscription_status: {str(e)}", exc_info=True)
        return {
...
async def start_trial(
    ...
    try:
        ...
        trial_end = int((datetime.utcnow().timestamp() + 7 * 24 * 60 * 60))  # 7 days from now
        
        # Create a customer in Stripe if they don't exist
        if not user_info.get('stripe_customer_id'):
    ...
    try:
        ...
        if subscription:
            ...
            if subscription.get('status') in ['active', 'trialing']:
                logger.error(f"Request came from an existing subscription: {user_id}")
        ...
        try:
            ...
            await store.user_repo.update_subscription_status(
                ...
                cancel_at_period_end=created_subscription.cancel_at_period_end,
