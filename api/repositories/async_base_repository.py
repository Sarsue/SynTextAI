...
class AsyncBaseRepository(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    """Base repository class with generic type support."""
    ...
    def __init__(self, manager: BaseRepositoryManager, session_factory):
        ...
        if self.model is None:
            raise ValueError("Repository subclass must set 'model' attribute")
    ...
    async def create(self, obj_in: Union[CreateSchemaType, Dict[str, Any]]) -> ModelType:
        async with self.session_scope() as session:
            if hasattr(obj_in, "model_dump"):
                ...
                raise ValueError("obj_in must be dict or pydantic model")
    ...
    async def get_multi(
        ...
        *,
        ...
        async with self.session_scope() as session:
            ...
            for key, value in filters.items():
                if hasattr(self.model, key):
                    ...
                    if value is None:
                        ...
                        count_stmt = count_stmt.where(col.in_(value))
                    else:
            ...
            if limit > 0 and offset == 0 and len(items) < limit:
                total = len(items)
            else:
        ...
        *,
        ...
        async with self.session_scope() as session:
            if hasattr(obj_in, "model_dump"):
                ...
                data = {k: v for k, v in dict(obj_in).items() if v is not None}
